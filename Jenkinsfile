pipeline{
  ...
  stages {
                        stage('Checkout') {
                            ...
                            url:'https://github.com/daniel-castro97/adn-alquiler.git'
                            ]]
                            ])
                            }
                        }
                        stage('Build project') {
                           steps {
                              sh 'gradle --b ./alquiler/build.gradle clean'
                              sh 'gradle --b ./alquiler/build.gradle build'
                           }
                        }
                        stage('Compile & Unit Tests') {
                           steps {
                              echo "--------------->Unit Tests<--------"
                              sh 'gradle --b ./alquiler/build.gradle test'
                              sh 'gradle --b ./alquiler/build.gradle jacocoTestReport'
                           }
                        }
                        ...
                        stage('Build') {
                           steps {
                              echo "-------->Build<---------"
                              sh 'gradle --b ./alquiler/build.gradle build -x test'
                           }
                        }         
         }
         post {
            ...
            success{
               ...
               junit 'alquiler/build/test-results/test/*.xml'
            }
            failure {
